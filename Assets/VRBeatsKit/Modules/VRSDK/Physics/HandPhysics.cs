...
namespace VRSDK
{
    public class HandPhysics
    {
        ...
        private Vector3 CalculateThrowDirection()
        {
            ...
            {
                throwDirection = (  localPositionHistory[localPositionHistory.Count - 1] - localPositionHistory[0] ).normalized;    
            }
            else
            {
...
#region MATH
        ...
        private float CalculateMagnitudeEMA(List<Vector3> buffer, float modifier)
        {
            ...
            if (buffer.Count > 0)
            {
                EMA = ( buffer[buffer.Count - 1].magnitude - SMA ) * ( smoothingConst * modifier );
            }
            EMA += SMA;
